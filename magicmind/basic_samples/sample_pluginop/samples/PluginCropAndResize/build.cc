...
void ConstructCropAndResizeNetwork(magicmind::DataType uint8_dt,
  ...
  auto model = SUniquePtr<magicmind::IModel>(
      builder->BuildModel("crop_and_resize_model", network.get(), config.get()));
  CHECK_VALID(model);
...
int main(int argc, char *argv[]) {
  if (argc != 2) {
    SLOG(INFO) << "plugin_crop_and_resize_test usage: ";
    SLOG(INFO)
        << "  ./${path_to_executable_dir}/plugin_crop_and_resize_op ${mm_plugin_so_dir_path} ";
    return -1;
  }
  ...
  std::string model_name = "crop_and_resize_model";
