...
int main(int argc, char **argv) {
  ...
  // create logical_xor node
  ILogicNode *LogicalXor = network->AddILogicNode(input1_tensor,input2_tensor,ILogic::XOR);

  // mark network output
  for (auto i = 0; i < LogicalXor->GetOutputCount(); i++) {
    auto output_tensor = LogicalXor->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("logical_xor_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("logical_xor_model"));
