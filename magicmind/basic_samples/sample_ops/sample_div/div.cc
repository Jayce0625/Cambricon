...
int main(int argc, char **argv) {
  ...
  // create div node
  IDivNode *Div = network->AddIDivNode(input1_tensor,input2_tensor);

  // mark network output
  for (auto i = 0; i < Div->GetOutputCount(); i++) {
    auto output_tensor = Div->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("div_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("div_model"));
