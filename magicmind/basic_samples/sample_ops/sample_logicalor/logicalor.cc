...
int main(int argc, char **argv) {
  ...
  // create logical_or node
  ILogicNode *LogicalOr = network->AddILogicNode(input1_tensor,input2_tensor,ILogic::OR);

  // mark network output
  for (auto i = 0; i < LogicalOr->GetOutputCount(); i++) {
    auto output_tensor = LogicalOr->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("logical_or_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("logical_or_model"));
