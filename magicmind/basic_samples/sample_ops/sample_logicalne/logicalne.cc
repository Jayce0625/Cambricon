...
int main(int argc, char **argv) {
  ...
  // create logical_ne node
  ILogicNode *LogicalNe = network->AddILogicNode(input1_tensor,input2_tensor,ILogic::NE);

  // mark network output
  for (auto i = 0; i < LogicalNe->GetOutputCount(); i++) {
    auto output_tensor = LogicalNe->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("logical_ne_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("logical_ne_model"));
