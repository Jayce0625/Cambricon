...
int main(int argc, char **argv) {
  ...
  // create floor mod node
  IFloorModNode *FloorMod = network->AddIFloorModNode(input1_tensor, input2_tensor);

  // mark network output
  for (auto i = 0; i < FloorMod->GetOutputCount(); i++) {
    auto output_tensor = FloorMod->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("floor_mod_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("floor_mod_model"));
