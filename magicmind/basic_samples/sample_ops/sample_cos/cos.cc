...
int main(int argc, char **argv) {
  ...
  // create cos node
  ICosNode *Cos = network->AddICosNode(input_tensor);

  // mark network output
  for (auto i = 0; i < Cos->GetOutputCount(); i++) {
    auto output_tensor = Cos->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("cos_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("cos_model"));
