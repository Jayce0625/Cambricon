...
int main(int argc, char **argv) {
  ...
  // create maximum node
  IMaximumNode *Maximum = network->AddIMaximumNode(input1_tensor,input2_tensor);

  // mark network output
  for (auto i = 0; i < Maximum->GetOutputCount(); i++) {
    auto output_tensor = Maximum->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("maximum_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("maximum_model"));
