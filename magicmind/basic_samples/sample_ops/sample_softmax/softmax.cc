...
int main(int argc, char **argv) {
  ...
  // create softmax node
  ISoftmaxNode *Softmax = network->AddISoftmaxNode(input1_tensor, input2_tensor);

  // mark network output
  for (auto i = 0; i < Softmax->GetOutputCount(); i++) {
    auto output_tensor = Softmax->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel(
      "softmax_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("softmax_model"));
