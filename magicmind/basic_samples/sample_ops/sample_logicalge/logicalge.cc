...
int main(int argc, char **argv) {
  ...
  // create logical_ge node
  ILogicNode *LogicalGe = network->AddILogicNode(input1_tensor,input2_tensor,ILogic::GE);

  // mark network output
  for (auto i = 0; i < LogicalGe->GetOutputCount(); i++) {
    auto output_tensor = LogicalGe->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("logical_ge_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("logical_ge_model"));
