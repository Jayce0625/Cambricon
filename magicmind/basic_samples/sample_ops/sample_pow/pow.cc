...
int main(int argc, char **argv) {
  ...
  // create pow node
  IPowNode *Pow = network->AddIPowNode(input1_tensor, input2_tensor);

  // mark network output
  for (auto i = 0; i < Pow->GetOutputCount(); i++) {
    auto output_tensor = Pow->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("pow_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("pow_model"));
