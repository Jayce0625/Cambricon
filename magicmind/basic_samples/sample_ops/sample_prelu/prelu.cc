...
int main(int argc, char **argv) {
  ...
  Dims input2_dims = Dims({1});
  magicmind::ITensor *input2_tensor = network->AddInput(input2_dtype, input2_dims);
  CHECK_VALID(input2_tensor);

  // create prelu node
  IPreluNode *Prelu = network->AddIPreluNode(input1_tensor, input2_tensor);

  // mark network output
  for (auto i = 0; i < Prelu->GetOutputCount(); i++) {
    auto output_tensor = Prelu->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("prelu_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("prelu_model"));
