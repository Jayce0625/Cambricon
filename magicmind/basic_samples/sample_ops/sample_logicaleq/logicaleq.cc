...
int main(int argc, char **argv) {
  ...
  // create logical_eq node
  ILogicNode *LogicalEq = network->AddILogicNode(input1_tensor,input2_tensor,ILogic::EQ);

  // mark network output
  for (auto i = 0; i < LogicalEq->GetOutputCount(); i++) {
    auto output_tensor = LogicalEq->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("logical_eq_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("logical_eq_model"));
