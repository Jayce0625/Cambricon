...
int main(int argc, char **argv) {
  ...
  // create abs node
  IAbsNode *Abs = network->AddIAbsNode(input_tensor);
  ...
  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("abs_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("abs_model"));
