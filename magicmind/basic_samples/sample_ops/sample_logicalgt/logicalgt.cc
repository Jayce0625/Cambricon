...
int main(int argc, char **argv) {
  ...
  // create logical_gt node
  ILogicNode *LogicalGt = network->AddILogicNode(input1_tensor,input2_tensor,ILogic::GT);

  // mark network output
  for (auto i = 0; i < LogicalGt->GetOutputCount(); i++) {
    auto output_tensor = LogicalGt->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("logical_gt_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("logical_gt_model"));
