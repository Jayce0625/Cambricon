...
int main(int argc, char **argv) {
  ...
  // create logical_lt node
  ILogicNode *LogicalLt = network->AddILogicNode(input1_tensor,input2_tensor,ILogic::LT);

  // mark network output
  for (auto i = 0; i < LogicalLt->GetOutputCount(); i++) {
    auto output_tensor = LogicalLt->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("logical_lt_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("logical_lt_model"));
