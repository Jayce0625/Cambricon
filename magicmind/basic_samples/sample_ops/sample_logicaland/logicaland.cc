...
int main(int argc, char **argv) {
  ...
  // create logical_and node
  ILogicNode *LogicalAnd = network->AddILogicNode(input1_tensor,input2_tensor,ILogic::AND);

  // mark network output
  for (auto i = 0; i < LogicalAnd->GetOutputCount(); i++) {
    auto output_tensor = LogicalAnd->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("logical_and_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("logical_and_model"));
