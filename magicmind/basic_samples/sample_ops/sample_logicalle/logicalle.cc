...
int main(int argc, char **argv) {
  ...
  // create logical_le node
  ILogicNode *LogicalLe = network->AddILogicNode(input1_tensor,input2_tensor,ILogic::LE);

  // mark network output
  for (auto i = 0; i < LogicalLe->GetOutputCount(); i++) {
    auto output_tensor = LogicalLe->GetOutput(i);
    CHECK_STATUS(network->MarkOutput(output_tensor));
  }

  // create model
  auto model = SUniquePtr<magicmind::IModel>(builder->BuildModel("logical_le_model", network.get()));
  CHECK_VALID(model);

  // save model to file
  CHECK_STATUS(model->SerializeToFile("logical_le_model"));
