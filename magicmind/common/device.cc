...
std::ostream &operator<<(std::ostream &out, const DeviceInfo &dev_info) {
  ...
  std::string ipu_clock_rate = dev_info.ipu_clock_rate_ < 0 ?
      "-" : std::to_string(dev_info.ipu_clock_rate_ / 1e6) + " (GHz)";
  out << std::setw(30) << std::left << "MLU Core Clock Rate: " << ipu_clock_rate << std::endl;
...
void InitDeviceEnv() {
// the cloud side device needs to initialize the cncontext before bind_cluster, and the cnrtmalloc
// can trigger the initialization of the cncontext
#ifndef MM_ENABLE_EDGE
  void *ptr = nullptr;
  cnrtMalloc(&ptr, 1);
  cnrtFree(ptr);
#endif
